...
//
//		HttpHeaders headers = new HttpHeaders();
//		headers.set("Authorization", String.format("Basic %s", new String(Base64.encode("my-trusted-client:".getBytes("UTF-8")), "UTF-8")));
//		headers.setAccept(Arrays.asList(MediaType.ALL));
//
//		@SuppressWarnings("rawtypes")
//		ResponseEntity<Map> response = serverRunning.postForMap("/FantaGameBE/app/oauth/token", headers, formData);
...
////		my-trusted-client-with-secret ok
//		MultiValueMap<String, String> formData = getTokenFormData("read", "my-trusted-client-with-secret");
//		ResponseEntity<Map> response = serverRunning.postForMap("/FantaGameBE/app/oauth/token", formData);
...
//		ResponseEntity<String> response = serverRunning.postForString("/FantaGameBE/app/oauth/token", formData);
//		assertEquals(HttpStatus.OK, response.getStatusCode());
//	}
...
//
//		// first make sure the resource is actually protected.
//		assertEquals(HttpStatus.UNAUTHORIZED, serverRunning.getStatusCode("/FantaGameBE/app/photos/user/message"));
//
//		// now make sure an authorized request is valid.
//		HttpHeaders headers = new HttpHeaders();
//		headers.set("Authorization", String.format("%s %s", OAuth2AccessToken.BEARER_TYPE, accessToken.getValue()));
//		assertEquals(HttpStatus.OK, serverRunning.getStatusCode("/FantaGameBE/app/secure/JSON/personServices/persona.json?user=cybertop", headers));
//	}
	...
	public void testUnsupportedMediaType() throws Exception {
		...
		// story.
		assertEquals(HttpStatus.OK, serverRunning.getStatusCode("/FantaGameBE/app/secure/JSON/personServices/test.json", headers));
	}
//
//	/**
...
//		headers.setAccept(Arrays.asList(MediaType.APPLICATION_XML));
//		assertEquals(HttpStatus.NOT_ACCEPTABLE, serverRunning.getStatusCode("/FantaGameBE/app/photos/user/message", headers));
//	}
	...
	private MultiValueMap<String, String> getTokenFormData(String scope, String clientId) {
		...
		formData.add("username", "cybertop");
		formData.add("password", "scarabeo");
		return formData;
	}
