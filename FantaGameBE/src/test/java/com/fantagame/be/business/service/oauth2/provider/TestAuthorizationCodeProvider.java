...
import com.fantagame.be.business.service.oauth2.provider.ServerRunning.UriBuilder;
...
public class TestAuthorizationCodeProvider {
	...
	public void testAuthorizationRequestRedirectsToLogin() throws Exception {
		...
		assertTrue(response.getBody().contains("/app/pub/login/login_security_check.do"));
	...
	public void testSuccessfulAuthorizationCodeFlow() throws Exception {
		...
		assertNotSame(HttpStatus.OK, serverRunning.getStatusCode("/FantaGameBE/app/secure/home.htm"));

		// now make sure an authorized request is valid.
		HttpHeaders headers = new HttpHeaders();
		headers.set("Authorization", String.format("%s %s", OAuth2AccessToken.BEARER_TYPE, accessToken.getValue()));
		assertEquals(HttpStatus.OK, serverRunning.getStatusCode("/FantaGameBE/app/secure/home.htm", headers));
	...
	public void testUserDeniesConfirmation() throws Exception {
		...
		ResponseEntity<Void> result = serverRunning.postForStatus("/FantaGameBE/app/oauth/authorize", headers, formData);
		assertEquals(HttpStatus.FOUND, result.getStatusCode());
	...
	public void testSuccessfulFlowWithRegisteredRedirect() throws Exception {
		...
		// first make sure the resource is actually protected.
		assertNotSame(HttpStatus.OK, serverRunning.getStatusCode("/FantaGameBE/app/photos?format=json"));

		// now make sure an authorized request is valid.
		HttpHeaders headers = new HttpHeaders();
		headers.set("Authorization", String.format("%s %s", OAuth2AccessToken.BEARER_TYPE, accessToken.getValue()));
		assertEquals(HttpStatus.OK, serverRunning.getStatusCode("/FantaGameBE/app/photos?format=json", headers));
	...
	public void testInvalidScopeInResourceRequest() throws Exception {
		...
		HttpHeaders headers = new HttpHeaders();
		headers.set("Authorization", String.format("%s %s", OAuth2AccessToken.BEARER_TYPE, accessToken.getValue()));
		ResponseEntity<String> response = serverRunning.getForString("/FantaGameBE/app/photos?format=json", headers);
	...
	public void testRegisteredRedirectWithWrongRequestedRedirect() throws Exception {
		...
		assertTrue(location.matches(serverRunning.getUrl("/FantaGameBE/app/login")+".*"));
	}
	...
	private String loginAndGetConfirmationPage(String clientId, String redirectUri, String scope) {
		...
		ResponseEntity<String> response = serverRunning.getForString(getAuthorizeUrl(clientId, redirectUri, scope),	headers);
	...
	private String getAuthorizeUrl(String clientId, String redirectUri, String scope) {
		UriBuilder uri = serverRunning.buildUri("/FantaGameBE/app/oauth/authorize").queryParam("response_type", "code")
	...
	private ResponseEntity<Map> requestToken(String clientId, String redirectUri, String cookie, String scope) {
		...
		ResponseEntity<Map> map = serverRunning.postForMap("/FantaGameBE/app/oauth/token", formData);
	...
	private String loginAndGrabCookie() {
		...
		ResponseEntity<Void> result = serverRunning.postForStatus("/FantaGameBE/app/pub/login/login_security_check.do", headers, formData);
		assertEquals(HttpStatus.FOUND, result.getStatusCode());
