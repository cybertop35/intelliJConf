...
public class ServerRunning implements MethodRule {
	...
	public Statement apply(final Statement base, FrameworkMethod method, Object target) {
		...
		try {
			client.getForEntity(new UriTemplate(getUrl("/FantaGameBE/app/pub/login.htm")).toString(), String.class);
			online = true;
			logger.info("Basic connectivity test passed");
		}
		catch (RestClientException e) {
			logger.warn(String.format(
					"Not executing tests because basic connectivity test failed for hostName=%s, port=%d", hostName,port), e);
			if (assumeOnline) {
		...
		return new Statement() {
			...
			public void evaluate() throws Throwable {
				try {
					postForStatus("/FantaGameBE/app/oauth/uncache_approvals", new LinkedMultiValueMap<String, String>());
					base.evaluate();
				}
				finally {
					postForStatus("/FantaGameBE/app/oauth/cache_approvals", new LinkedMultiValueMap<String, String>());
				}
